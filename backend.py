...
class Database:
    ...
    def view(self): # fetches all rows
        ...
        rows=self.cur.fetchall()  # 'rows' is a local variable that needs no self prefix
        # self.conn.close() closing the db causes an error. Odd--why wasn't this an issue before?
        return rows
